...
For this dataset, a hit is defined as a song that featured in the weekly list (Issued by Billboards) of Hot-100 tracks in the decade at least once. A flop is defined with the following conditions:
    - track must not appear in the hit list of the decade
    - track's artist must not appear in the hit list of the decade
    - track must belong to a genre that is not mainstream
    - track's genre must not appear in the hit list of the decade
    - track must have US as one of its markets

The following is a list of columns in the data:
    - track
    - artist
    ...
    - chorus_hit (when the chorus starts)
    - sections
    - target (if the track is a hit or flop)
...
There are two main packages that are useful in the creation of a decision tree. These are the {tree} and {rpart} (recursive partitioning) packages. For this algorithm, we will be using {rpart} because it provides more flexibility for surrogate splits.
...
We used {rpart.plot} in order to create the visualization for the tree. {rpart.plot} can only be used to the {rpart} class of objects. It will yield the Collection Node style of trees. 
```{r}
# Display rpart.plot ----
# library(rpart.plot)
rpart.plot(
...
```
This is a matrix that shows the metrics of our model. Accuracy shows how often our model is correct, sensitivity measures how well our model identifies positives, and
specificity shows how well our model identifies negatives. These results show that the model has a tendency to incorrectly label tracks as hits even when they are not. However, it is good at identifying tracks that are hits. 
